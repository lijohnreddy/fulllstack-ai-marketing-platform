...
function PromptEditorDialog({
  ...
  useEffect(() => {
    setIsExceeded(currentTokenCount > MAX_TOKENS_PROMPTS);
  }, [currentTokenCount]);
      ...
      <DialogContent className="max-w-[90%] sm:max-w-[80%] lg:max-w-[40%] bg-white p-8 md:px-12 md:py-8 rounded-3xl sm:rounded-3xl lg:border-4 border-main space-y-1">
        <div className="space-y-1">
          <div className="flex items-center space-x-2">
            <MessageSquare
              className="h-7 w-7 sm:h-8 sm:w-8 text-main shrink-0"
              strokeWidth={3}
            />
            <Input
              name="name"
              ...
              className="grow font-semibold text-xl sm:text-2xl text-main bg-transparent border-none p-0 placeholder:text-main/75 focus:outline-none focus-visible:ring-0 focus-visible:ring-offset-0"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center space-y-2 sm:space-y-0">
          ...
          >
            ...
            {formatTokens(MAX_TOKENS_PROMPTS)}
          </span>
