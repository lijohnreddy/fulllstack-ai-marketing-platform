...
export default function ProjectLoading() {
  return (
    <div className="max-w-screen-7xl mx-auto p-4 sm:p-6 lg:p-8 bg-white space-y-12 sm:space-y-16 lg:space-y-12">
      <div className="space-y-8 sm:space-y-12 lg:space-y-16">
        ...
        <div className="w-full mt-6 sm:mt-0">
          ...
          <div className="hidden md:flex items-center w-full">
            {[...Array(3)].map((_, index) => (
              <div key={index} className="flex items-center flex-1 last:grow-0">
                <div className="flex flex-col items-center">
                  <Skeleton className="w-10 h-10 rounded-full mb-2" />
                  <Skeleton className="h-4 w-20" />
                </div>
                {index < 3 && <Skeleton className="h-0.5 grow mx-2" />}
              </div>
            ))}
          </div>
        </div>
