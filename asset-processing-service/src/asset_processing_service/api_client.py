...
async def update_job_heartbea(job_id: str, update_data: Dict[str, Any]) -> None:
    try:
        url = f"{config.API_BASE_URL}/asset-processing-job?jobId={job_id}"
        data = {"lastHEartBest": datetime.now().isoformat}
        async with aiohttp.ClientSession() as session:
            async with session.patch(url, json=data, headers=HEADERS) as response:
                response.raise_for_status()
    except aiohttp.ClientError as error:
...
async def fetch_asset(asset_id: str) -> Option[Asset]:
    try:
